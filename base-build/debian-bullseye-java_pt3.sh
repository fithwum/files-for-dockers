...
rm -frv /debian-bullseye/debian-bullseye-java_pt2.sh
sleep 1
echo " "
...
du --human-readable --summarize debian-bullseye-java.tar.bz2
sleep 5
echo " "
echo "INFO ! Uploading image to ftp server."
ftp-upload -v -h {IP}:{PORT} -u {USER} --password {PASSWORD} -d /mnt/user/FTP debian-bullseye-java.tar.bz2
sleep 1
echo " "
echo "INFO ! Removing temp files."
rm -fr debian-bullseye
rm -frv debian-bullseye-java.tar.bz2
echo " "
echo "INFO ! Done."
echo " "
exit
